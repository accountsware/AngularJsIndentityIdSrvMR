...
using Angular.AuthInfrastructure;
...
using ClaimsExtensions = BrockAllen.MembershipReboot.ClaimsExtensions;
...
namespace Angular.AuthServer.UserManagementExtension
{
        ...
        
    {
        ...
        protected virtual Task<AuthenticateResult> PostAuthenticateLocalAsync(UserAccount account, SignInMessage message)
        {
            return Task.FromResult<AuthenticateResult>(null);
        }

        protected virtual bool ValidateLocalCredentials(string username, string password, SignInMessage message, out UserAccount account)
        {
        ...
        protected virtual Task<IEnumerable<Claim>> GetClaimsForAuthenticateResult(UserAccount account)
        {
        ...
        protected virtual Task<UserAccount> InstantiateNewAccountFromExternalProviderAsync(string provider, string providerId, IEnumerable<Claim> claims)
        {
        ...
        protected virtual async Task<AuthenticateResult> SignInFromExternalProviderAsync(Guid accountID, string provider)
        {
            ...
            return new AuthenticateResult(
                ...
                authenticationMethod: Angular.AuthInfrastructure.Constants.AuthenticationMethods.External);
        }
        ...
        {
            var email = ClaimsExtensions.GetValue(claims, Constants.ClaimTypes.Email);
            if (email != null)
            {
                var acct = userAccountService.GetByID(accountID);
                if (acct.Email == null)
                {
                    try
                    {
                        var email_verified = ClaimsExtensions.GetValue(claims, Constants.ClaimTypes.EmailVerified);
