...
namespace BrockAllen.MembershipReboot
{
      ...
      
    {
        ...
        EventBusUserAccountRepository<UserAccount> userRepository;

        Lazy<AggregateValidator> usernameValidator;
        Lazy<AggregateValidator> emailValidator;
        Lazy<AggregateValidator> passwordValidator;

        public UserAccountService(IUserAccountRepository<UserAccount> userRepository)
            : this(new MembershipRebootConfiguration<UserAccount>(), userRepository)
        {
        }

        public UserAccountService(MembershipRebootConfiguration<UserAccount> configuration, IUserAccountRepository<UserAccount> userRepository)
        {
            ...
            this.userRepository = new EventBusUserAccountRepository<UserAccount>(this, userRepository,
            ...
            {
                var val = new AggregateValidator();
                val.Add(UserAccountValidation.EmailIsRequiredIfRequireAccountVerificationEnabled);
        ...
        protected void Init(UserAccount account, string tenant, string username, string password, string email, Guid? id = null, DateTime? dateCreated = null)
        {
            ...
            this.AddEvent(new AccountCreatedEvent { Account = account, InitialPassword = password, VerificationKey = key });
        }
        ...
        public virtual void RequestAccountVerification(Guid accountID)
        {
            ...
            this.AddEvent(new EmailChangeRequestedEvent { Account = account, NewEmail = account.Email, VerificationKey = key });
        ...
        public virtual void ReopenAccount(UserAccount account)
        {
            ...
            ClearVerificationKey(account);
            var key = SetVerificationKey(account, VerificationKeyPurpose.ChangeEmail, state: account.Email);
            this.AddEvent(new AccountReopenedEvent { Account = account, VerificationKey = key });
            ...
            if (result)
            {
                try
                {
                    ...
                    if (account.IsAccountClosed)
                    {
                        ...
                        this.AddEvent(new InvalidAccountEvent { Account = account });
        ...
        public virtual void VerifyEmailFromKey(string key, string password, out UserAccount account)
        {
            ...
            this.AddEvent(new EmailVerifiedEvent { Account = account });
