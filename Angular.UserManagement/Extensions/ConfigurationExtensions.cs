...
namespace BrockAllen.MembershipReboot
{
    public static class ConfigurationExtensions
    {
        ...
        public static void RegisterUsernameValidator<TAccount>(this MembershipRebootConfiguration<TAccount> config, Func<UserAccountService, TAccount, string, ValidationResult> func)
            where TAccount : UserAccount
        {
            if (config == null) throw new ArgumentNullException("config");
            config.RegisterUsernameValidator(new DelegateValidator<TAccount>(func));
        }
        public static void RegisterPasswordValidator<TAccount>(this MembershipRebootConfiguration<TAccount> config, Func<UserAccountService, TAccount, string, ValidationResult> func)
            where TAccount : UserAccount
        {
            if (config == null) throw new ArgumentNullException("config");
            config.RegisterPasswordValidator(new DelegateValidator<TAccount>(func));
        }
        public static void RegisterEmailValidator<TAccount>(this MembershipRebootConfiguration<TAccount> config, Func<UserAccountService, TAccount, string, ValidationResult> func)
            where TAccount : UserAccount
        {
            if (config == null) throw new ArgumentNullException("config");
